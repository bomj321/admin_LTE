...
include('conexion.php');
 ...
 $sql=("SELECT e.*,p.*,c.*, pl.* FROM empleados as e inner join provincias as p on (p.id_provi=e.Provincia) inner join cantones as c on (c.id_canto=e.Canton) inner join planes as pl on (pl.id_plan=e.Plan) WHERE e.Cedula='$n'
");
...
<body class="hold-transition skin-blue sidebar-mini" onload="provi('0')" >
<div class="wrapper">
...
$insert = mysqli_query($con, "UPDATE empleados SET Cedula='$Cedula',numero_cliente='$Numero_Cliente', Nombre='$Nombre', Telefono='$Telefono',Email='$Email',Provincia='$Provincia',Canton='$Canton',Tiempo='$Tiempo',Bloque='$Bloque',Plan='$Plan',ClaveATV='$ClaveATV',img_emple='$image' WHERE Cedula='$Id'
") or die(mysqli_error());
}
else{
         $insert = mysqli_query($con, "UPDATE empleados SET Cedula='$Cedula',numero_cliente='$Numero_Cliente', Nombre='$Nombre', Telefono='$Telefono',Email='$Email',Provincia='$Provincia',Canton='$Canton',Tiempo='$Tiempo',Bloque='$Bloque',Plan='$Plan',ClaveATV='$ClaveATV' WHERE Cedula='$Id'
") or die(mysqli_error());         
}
            if($insert){
...
while($datos=mysqli_fetch_array($user)){ 
        ...
        <div class="form-group">
          ...
          <div class="col-sm-4">
            <input type="text" value="<?php  echo  $datos['numero_cliente'];?>" name="Numero_Cliente" class="form-control" placeholder="Numero del Cliente" required>
          </div>
        </div>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-4">
            <input type="text" value="<?php  echo  $datos['Cedula'];?>" name="Cedula" class="form-control" placeholder="Cedula" required>
          </div>
        </div>
        <div class="form-group">
          <label class="col-sm-3 control-label">Nombre Completo</label>
          <div class="col-sm-4">
            <input type="text" name="Nombre" value="<?php  echo  utf8_encode($datos['Nombre']);?>" class="form-control" placeholder="Nombre" required>
          </div>
        </div>
        <div class="form-group">
          <label class="col-sm-3 control-label">Teléfono</label>
          <div class="col-sm-4">
            <input type="text" name="Telefono" value="<?php  echo  $datos['Telefono'];?>" class="form-control" placeholder="Telefono" required>
          </div>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-4">
            <input type="email" name="Email" value="<?php  echo  $datos['Email'];?>" class="form-control" placeholder="Email" required>
          </div>
            </div>
        <div class="form-group">
          ...
          <div class="col-sm-4">
            ...
            <option value="<?php  echo  $datos['Provincia'];?>"><?php  echo  utf8_encode($datos['nom_provi']);?></option>
            <option value="">......</option>
             <?php
        while($datos1=mysqli_fetch_array($provi)){ ?>
              <option value="<?php  echo  $datos1['id_provi'];?>"><?php  echo  utf8_encode($datos1['nom_provi']);?></option>
              <?php } ?>
            </select>
          </div>
            </div>
        <div class="form-group">
          <label class="col-sm-3 control-label">Cantón</label>
...
$nom_ti="Anual";
              ...
              ?>
              <option value="<?php  echo  $datos['Tiempo'];?>"><?php  echo  $nom_ti;?></option>
...
<option value="<?php  echo  $datos['Bloque'];?>">Bloque<?php  echo  $datos['Bloque'];?></option>
...
<div class="form-group">
    ...
    <option value="<?php  echo  $datos['id_plan'];?>"><?php  echo  utf8_encode($datos['nom_plan']);?></option>
            <option value="">......</option>
             <?php
...
<div class="form-group">
      ...
      <p class="btn btn-sm btn-danger" id="b-credi" onclick="mostrar()">Editar Credenciales</p>
                     <label class="btn btn-sm btn-danger" style="display: none;" id="btcredi" onclick="mostrart()">X</label>

          <div class="col-sm-4" style="display: none;" id="credi">
...
<?php } ?>
 <!-- /.FIN DE LA LINEA DE USO -->
  ...
  <footer class="main-footer">
    ...
    <strong>Yoward Diaz/Admin</a>.</strong>
  </footer>
