<?php
    ...
    while($rows = $resultado->fetch_assoc()){
        ...
        if ($rows['Bloque']==5) {
            $rows['Bloque'] ='Pendiente';
        }elseif ($rows['Bloque']==6) {
           $rows['Bloque'] ='Cancelado';
        }elseif ($rows['Bloque']==1) {
           $rows['Bloque'] ='1-7';
        }elseif ($rows['Bloque']==2) {
           $rows['Bloque'] ='8-15';
        }elseif ($rows['Bloque']==3) {
           $rows['Bloque'] ='16-23';
        }elseif ($rows['Bloque']==4) {
           $rows['Bloque'] ='23-31';
        }
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('F'.$fila, $rows['nom_provi']);
        $objPHPExcel->getActiveSheet()->setCellValue('G'.$fila, $rows['nom_canto']);
        $objPHPExcel->getActiveSheet()->setCellValue('H'.$fila, $rows['Tiempo']);
        $objPHPExcel->getActiveSheet()->setCellValue('I'.$fila, $rows['nom_plan']);
